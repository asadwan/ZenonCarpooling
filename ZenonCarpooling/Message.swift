    ...
    class func send(message: Message, toID: String, completion: @escaping (Bool) -> Swift.Void)  {
        if let currentUserID = Auth.auth().currentUser?.uid {
            ...
            case .photo:
                ...
                let messagePicRef = Storage.storage().reference().child("messagePics").child(child)
                messagePicRef.putData(imageData!, metadata: nil, completion: { (metadata, error) in
                    if error == nil {
                        messagePicRef.downloadURL(completion: { (url, error) in
